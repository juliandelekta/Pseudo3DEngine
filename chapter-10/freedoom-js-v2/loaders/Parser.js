const Parser = {
    ...
    parseSector(name, info) {
        ...
        sector.light = info.light**2 / 65536
        sector.floor = this.parseFloor(info.floor)
        sector.ceiling = this.parseCeiling(info.ceiling)

        if (info.loops) {
            sector.border = this.parseLoop(info.loops[0])
            sector.segments.push(...sector.border)
            for (let i = 1; i < info.loops.length; i++) {
                sector.segments.push(...this.parseLoop(info.loops[i]))
                const next = info.loops[i].walls.filter(x => x.next)
                if (next.length) {
                    const subsectors = new Set(next.map(x => x.next))
                    sector.subsectors.push(...subsectors.values())
                }
            }
        }
    ...
    parseCeiling(info) {
        ...
        if (info.texture.slice(0,2) === "f_") {
            info.texture = "p-" + info.texture.slice(2)
            ceil.parallax = true
        }
