const Portal = () => ({
    ...
    clipping() {
        ...
        this.lowerLightmap.u0 = .5 + this.segment.p0.l * (this.lowerLightmap.w-2)
        this.lowerLightmap.u1 = .5 + this.segment.p1.l * (this.lowerLightmap.w-2)
        this.upperLightmap.u0 = .5 + this.segment.p0.l * (this.upperLightmap.w-2)
        this.upperLightmap.u1 = .5 + this.segment.p1.l * (this.upperLightmap.w-2)
    ...
    draw(viewport) {
        ...
        if (this.next.floor.z > bottomZ) {
            ...
            this.drawWithLight(this.lower, this.lowerLightmap, viewport)
        ...
        if (this.next.ceiling.z < topZ) {
            ...
            this.drawWithLight(this.upper, this.upperLightmap, viewport)
        }
    ...
    drawLightmap(lightmap, viewport, topFactor = 1, bottomFactor = 0) {
        const s = this.segment
        ...
        let v = (y - top * topFactor - bottom * bottomFactor - 1) * dv + .5
    ...
    drawWithLight(texture, lightmap, viewport, topFactor = 1, bottomFactor = 0) {
        const s = this.segment
        ...
        let v = texture.offV + (y - top * topFactor - bottom * bottomFactor - 1) * dv
        let vl = (y - top * topFactor - bottom * bottomFactor - 1) * dvl + .5
        ...
        for (y *= 4; y < b; y+=4, v+=dv, vl+=dvl) {
            ...
            const l = Math.min((Q11 + (ul - ~~ul) * (Q21 - Q11) + (vl - gv) * (Q12 - Q11 + (ul - ~~ul) * (Q22 - Q12 - Q21 + Q11))) * .003921 + this.segment.sector.light, 1)
